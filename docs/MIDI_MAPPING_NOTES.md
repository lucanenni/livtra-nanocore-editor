...
**Framing note**: SysEx isn't a "workaround" for something CC can't do — SysEx is the pedal's
actual patch-programming protocol (how a patch's settings are edited/stored), while MIDI CC exists
for *live* control while playing. AMP/CAB model selection and chain reordering are programming-
style actions, not live-performance controls, so it makes sense they're SysEx's job — CC43/44
existing at all (and not working) is closer to a documentation artifact than a real regression.
Below, "instead of CC"/"SysEx-only" describe which wire messages actually carry the command, not
a hierarchy where one is the intended path and the other a fallback for it.
...
**2026-09-19: a NanoCore firmware update's release notes claimed "Fixed MIDI CC 43 and CC 44 not
responding for AMP and IR switching" — confirmed real on a firmware-updated device.** Sent raw
CC43=6 then CC44=6 directly (bypassing this editor's own SysEx path, via a throwaway script) —
the device's own screen changed to AMP=Hiw103 and CAB=Hiw412 as expected, both confirmed by the
user watching the device. **This does not change anything about how this editor itself talks to
the device — it still uses `sysexTypeField`/`buildBlockTypeSysEx` unconditionally for AMP/CAB, and
that stays correct** (SysEx is this device's real patch-programming protocol, not a workaround for
CC — see the framing note above; it already worked on every firmware version, this update didn't
change that). The practical upshot of the fix is for *external* live control: on updated firmware,
a plain MIDI foot controller or CC-only device can now also select AMP/CAB models directly, which
previously needed SysEx-aware software — CC's proper job (live control) now actually covers this
action too, on updated firmware. **Not re-tested by this fix: `0x63`'s AMP/CAB `variant` byte
still does not move** (confirmed via an automated before/after `0x63` diff around the same CC43/44
sends — only the sequence-number byte changed, nothing else) — the live-overlay carve-out in
`patchStore.ts` that keeps the saved-state (`0x41`) variant for AMP/CAB specifically (see the
"AMP/CAB model-index in 0x63" entry in [[nanocore-readback-pending]]) is unaffected by this
firmware update and remains necessary. Firmware version number not established (the release notes
didn't state one, and this device's on-device version display wasn't checked as part of this
test).
