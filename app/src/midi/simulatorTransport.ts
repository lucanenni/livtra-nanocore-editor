...
export class SimulatorTransport implements MidiTransport {
  ...
  /** No real device to read from — never calls `cb`. Callers reading the current patch back
   * (see `midi/presetReader.ts`) should expect this to simply time out for the simulator. */
  onSysExReceived(): () => void {
    return () => {};
  }
}
