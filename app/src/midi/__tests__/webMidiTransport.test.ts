...
describe('WebMidiTransport', () => {
  ...
  it('requests sysex access (needed for AMP/CAB model select and chain reorder)', async () => {
    const fakeOut = makeFakeOutput('port-1', 'Test Port');
    const requestMIDIAccess = vi.fn().mockResolvedValue(makeFakeAccess([fakeOut]));
    (navigator as any).requestMIDIAccess = requestMIDIAccess;
  ...
  it('sends raw SysEx bytes verbatim via MIDIOutput.send', async () => {
    const fakeOut = makeFakeOutput('port-1', 'Test Port');
    (navigator as any).requestMIDIAccess = vi.fn().mockResolvedValue(makeFakeAccess([fakeOut]));
    const t = new WebMidiTransport();
    await t.init();

    const bytes = [0xf0, 0x7d, 0x4e, 0x43, 0x70, 0x00, 0x02, 0x01, 0x00, 0x6d, 0x00, 0x02, 0x00, 0x00, 0x06, 0x0f, 0xf7];
    t.sendSysEx('port-1', bytes, 'AMP type -> Pey51501');

    expect(fakeOut.send).toHaveBeenLastCalledWith(bytes);
  });
