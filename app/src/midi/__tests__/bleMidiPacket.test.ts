...
describe('encodeBleMidiSysEx', () => {
  it('fits a short SysEx (AMP/CAB select, 17 bytes) in a single packet', () => {
    const sysex = [0xf0, 0x7d, 0x4e, 0x43, 0x70, 0x00, 0x02, 0x01, 0x00, 0x6d, 0x00, 0x02, 0x00, 0x00, 0x06, 0x0f, 0xf7];
    const packets = encodeBleMidiSysEx(sysex, 0);
    expect(packets).toHaveLength(1);
    expect(Array.from(packets[0])).toEqual([0x80, 0x80, ...sysex]);
  });

  it('splits a SysEx too long for one packet (chain reorder, 26 bytes) into consecutive packets', () => {
    const sysex = Array.from({ length: 26 }, (_, i) => i & 0x7f);
    const packets = encodeBleMidiSysEx(sysex, 0);
    expect(packets.length).toBeGreaterThan(1);

    // Reassembling every packet's data bytes (after the 2-byte header) reproduces the original.
    const reassembled = packets.flatMap((p) => Array.from(p.slice(2)));
    expect(reassembled).toEqual(sysex);

    // Every packet stays within the 20-byte default ATT payload.
    for (const p of packets) expect(p.length).toBeLessThanOrEqual(20);
  });
});
