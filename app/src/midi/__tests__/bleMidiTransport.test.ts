...
describe('BleMidiTransport', () => {
  ...
  it('sends a short SysEx (fits one BLE-MIDI packet) as raw framed bytes', async () => {
    const characteristic = makeFakeCharacteristic();
    const device = makeFakeDevice(characteristic);
    (navigator as any).bluetooth = { requestDevice: vi.fn().mockResolvedValue(device), getAvailability: vi.fn() };
    ...
    const t = new BleMidiTransport();
    await t.init();
    const sysex = [0xf0, 0x7d, 0x4e, 0x43, 0x70, 0x00, 0x02, 0x01, 0x00, 0x6d, 0x00, 0x02, 0x00, 0x00, 0x06, 0x0f, 0xf7];
    t.sendSysEx('ble-1', sysex);
    await new Promise((r) => setTimeout(r, 0)); // let the internal write queue flush

    expect(characteristic.writeValueWithoutResponse).toHaveBeenCalledTimes(1);
    const packet = characteristic.writeValueWithoutResponse.mock.calls[0][0] as Uint8Array;
    expect(Array.from(packet.slice(2))).toEqual(sysex);
  });

  it('splits a long SysEx (chain reorder) across multiple in-order BLE-MIDI packets', async () => {
    const characteristic = makeFakeCharacteristic();
    const device = makeFakeDevice(characteristic);
    (navigator as any).bluetooth = { requestDevice: vi.fn().mockResolvedValue(device), getAvailability: vi.fn() };
    ...
    const t = new BleMidiTransport();
    await t.init();
    // NanoCore's chain-order SysEx is ~26 bytes — exceeds one default-MTU BLE-MIDI packet.
    const sysex = [0xf0, 0x7d, 0x4e, 0x43, 0x70, 0x00, 0x02, 0x01, 0x00, 0x6d, 0x00, 0x0a, 0x00, 0x00, 0x05, 0x08, 0x00, 0x01, 0x07, 0x02, 0x03, 0x00, 0x04, 0x05, 0x06, 0xf7];
    t.sendSysEx('ble-1', sysex);
    await new Promise((r) => setTimeout(r, 0)); // let the chained writes flush

    expect(characteristic.writeValueWithoutResponse.mock.calls.length).toBeGreaterThan(1);
    const reassembled = characteristic.writeValueWithoutResponse.mock.calls.flatMap((call: unknown[]) =>
      Array.from((call[0] as Uint8Array).slice(2)),
    );
    expect(reassembled).toEqual(sysex);
  });

  it('emits a single sysex OutgoingMessage with the full byte array, regardless of packet count', async () => {
    const characteristic = makeFakeCharacteristic();
    const device = makeFakeDevice(characteristic);
    (navigator as any).bluetooth = { requestDevice: vi.fn().mockResolvedValue(device), getAvailability: vi.fn() };
    ...
    const sysex = [0xf0, 0x7d, 0x4e, 0x43, 0x70, 0x00, 0x02, 0x01, 0x00, 0x6d, 0x00, 0x02, 0x00, 0x00, 0x07, 0x00, 0xf7];
    t.sendSysEx('ble-1', sysex, 'CAB type -> Bog412A');

    expect(received).toEqual([expect.objectContaining({ kind: 'sysex', bytes: sysex, description: 'CAB type -> Bog412A' })]);
  });
