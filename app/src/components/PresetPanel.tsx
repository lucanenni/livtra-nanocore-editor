...
export function PresetPanel() {
  ...
  const saveToDeviceSlot = usePatchStore((s) => s.saveToDeviceSlot);
  ...
  const [saveSlotNumber, setSaveSlotNumber] = useState(0);
  ...
  return (
    <section className="preset-panel">
      ...
      <div className="preset-panel__recall">
        <h3 className="panel-subtitle">{t('preset.renameTitle', 'Rename patch on device')}</h3>
        <p className="panel-hint">
          {t(
            'preset.renameHint',
            "Renames the device's currently active/edited patch (live, like any other tweak — up to 8 characters, the device's real limit). This does NOT persist on its own: use Save to device slot below afterward, or it's lost on the next preset change.",
          )}
        </p>
        <div className="preset-panel__recall-row">
          <input
            ...
            placeholder={activeSlotName ?? undefined}
            aria-label={t('preset.renamePlaceholder', 'New patch name (max 8 characters)') ?? ''}
          />
          <button
            type="button"
            className="btn btn--small"
            disabled={!connectionReady || renameValue.trim().length === 0}
            onClick={() => {
              renamePresetOnDevice(renameValue);
              setRenameValue('');
            }}
          >
            {t('preset.renameButton', 'Rename')}
          </button>
        </div>
      </div>

      <div className="preset-panel__recall">
        <h3 className="panel-subtitle">{t('preset.saveTitle', 'Save to device slot')}</h3>
        <p className="panel-hint">
          {t(
            'preset.saveHint',
            'Saves whatever the device currently has loaded (recalled preset plus any live tweaks) into a slot — it does NOT first push this editor\'s own patch to the device. Send the patch first if you want to save what you see here. Same 0-indexed / +1-on-the-device numbering as recall above.',
          )}
        </p>
        <div className="preset-panel__recall-row">
          <input
            ...
            min={0}
            max={127}
            value={saveSlotNumber}
            onChange={(e) => setSaveSlotNumber(Number(e.target.value))}
            aria-label={t('preset.saveSlotNumber', 'Save slot number (0-127)') ?? ''}
          />
          <button
            type="button"
            className="btn btn--small"
            ...
            onClick={async () => {
              setSaving(true);
              setSaveStatus(null);
              const ok = await saveToDeviceSlot(saveSlotNumber);
              setSaving(false);
              setSaveStatus(
                ok
                  ? t('preset.saveOk', 'Saved to slot {{n}}.', { n: saveSlotNumber + 1 }) ?? null
                  : t('preset.saveFailed', 'No response from the device — save may not have happened.') ?? null,
              );
            }}
          >
            {saving ? t('preset.saving', 'Saving…') : t('preset.saveToSlotButton', 'Save')}
          </button>
        </div>
        {saveStatus && <p className="panel-hint">{saveStatus}</p>}
      </div>
    </section>
  );
}
