          ...
          {log.map((msg, i) => (
            <li key={i} className="activity-log__item">
              <span className="activity-log__tag">
                {msg.kind === 'cc'
                  ? t('log.cc', 'CC{{cc}}', { cc: msg.cc })
                  : msg.kind === 'pc'
                    ? t('log.pc', 'PC')
                    : t('log.sysex', 'SysEx')}
              </span>
              <span
                className="activity-log__value"
                title={msg.kind === 'sysex' ? msg.bytes?.map((b) => b.toString(16).padStart(2, '0')).join(' ') : undefined}
              >
                {msg.kind === 'cc'
                  ? msg.value
                  : msg.kind === 'pc'
                    ? msg.program
                    : t('log.sysexBytes', '{{count}}B', { count: msg.bytes?.length ?? 0 })}
              </span>
              <span className="activity-log__desc">{msg.description}</span>
              {msg.kind !== 'sysex' && <span className="activity-log__channel">ch{msg.channel}</span>}
            </li>
          ))}
        </ul>
      )}
