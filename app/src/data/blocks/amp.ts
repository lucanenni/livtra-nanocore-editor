...
export const amp: BlockSpec = {
  ...
  sysexTypeField: 0x06,
  note:
    'Model selection is confirmed on real hardware to NOT work over the documented typeCC (43) — ' +
  ...
  commonParams: [
    ...
    // docs/MIDI_MAPPING_NOTES.md's "Editor-assigned default values" entry) — gain resets to
    // maximum, not a middling value, on a truly blank slot.
    range('gain', 60, 0, 1, { decimals: 2, default: 1 }),
    range('bass', 61, -10, 10, { unit: 'dB', default: 0 }),
    range('mid', 62, -10, 10, { unit: 'dB', default: 0 }),
    range('treble', 63, -10, 10, { unit: 'dB', default: 0 }),
    // Manual lists Level (CC64) alongside Bass/Mid/Treble as a ±10 dB trim, but a real device
    // reading found it's a plain 0-100 level control, not a dB trim — see docs/PARAM_VERIFICATION.md.
    range('level', 64, 0, 100, { default: 80 }),
  ],
  types,
};
