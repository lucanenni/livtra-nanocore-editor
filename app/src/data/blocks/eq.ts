...
// No CC in the MIDI guide for this one — confirmed present on the device screen for every EQ
// type (real range -12~+12 dB, PARAM_VERIFICATION.md), sent via the SysEx per-parameter
// mechanism instead (see midi/sysex.ts's SET_FIELD_OPCODE doc comment). Its sysexParamIndex is
// the band count, since it must come right after that type's own band params in the array — see
// rangeSysexOnly's doc comment.
const level = (bandCount: number) => rangeSysexOnly('level', bandCount, -12, 12, { unit: 'dB', default: 0 });
