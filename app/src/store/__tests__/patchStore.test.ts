...
describe('patchStore.sendTestCC', () => {
  it('sends exactly the raw CC/value given, independent of any param scaling', () => {
    usePatchStore.getState().sendTestCC(43, 6, 'Test: AMP — Type / Model select = 6 (CC43=6)');
    const log = usePatchStore.getState().log;
    expect(log).toHaveLength(1);
    expect(log[0]).toMatchObject({ kind: 'cc', cc: 43, value: 6, description: 'Test: AMP — Type / Model select = 6 (CC43=6)' });
  });

  it('falls back to a generic description when none is given', () => {
    usePatchStore.getState().sendTestCC(80, 127);
    expect(usePatchStore.getState().log[0]?.description).toBe('Test CC80 = 127');
  });

  it('is a no-op with no connected output, like every other send action', () => {
    usePatchStore.getState().disconnectDevice();
    usePatchStore.getState().sendTestCC(80, 127);
    expect(usePatchStore.getState().log).toHaveLength(0);
  });
