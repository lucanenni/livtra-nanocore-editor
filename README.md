...
over MIDI, with one confirmed exception: **AMP and CAB model selection has no
effect over the documented CC protocol.** The manual's *ranges* for individual
...
[`docs/PARAM_VERIFICATION.md`](docs/PARAM_VERIFICATION.md). They work over SysEx instead — not
documented anywhere, but reverse-engineered from real traffic between the
official Livtra app and a real NanoCore (same hardware-verification approach
as everything else in this project, just applied to SysEx). Effect chain
